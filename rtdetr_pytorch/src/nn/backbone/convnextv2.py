...
# 直接从Convnextv2官方提供的ImageNet-1K fine_tuned models
download_url = {
...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(self, in_chans=3, return_idx=[0, 1, 2, 3], num_classes=1000, variant='convnextv2_base',
                 drop_path_rate=0., head_init_scale=1., pretrained=False
                 ):
        super().__init__()
        ...
        if pretrained and variant in download_url:
            state = torch.hub.load_state_dict_from_url(download_url[variant])
            if 'model' in state:
                state = state['model']
            # print('预训练权重参数名:', state.keys())
            self.load_state_dict(state, strict=False)
            print(f'Load ConvNextv2 {variant} state_dict')
    ...
    #         x = self.stages[i](x)
    #     return self.norm(x.mean([-2, -1]))  # global average pooling, (N, C, H, W) -> (N, C) 这个是分类任务专用
    #
    # def forward(self, x):
