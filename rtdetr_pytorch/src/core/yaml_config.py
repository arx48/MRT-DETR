...
class YAMLConfig(BaseConfig):
    def __init__(self, cfg_path: str, **kwargs) -> None:
        ...
        merge_dict(cfg, kwargs)     # 用外部传入的参数更新这个配置字典
        ...
        self.offset_pretrain = cfg.get('offset_pretrain', None)
        self.freeze_offset = cfg.get('freeze_offset', False)
    ...
    def model(self, ) -> torch.nn.Module:
        if self._model is None and 'model' in self.yaml_cfg:
            ...
            self._model = create(self.yaml_cfg['model'])    # 工厂方法，根据配置中指定的模型结构自动生成pytorch实例：create('RTDETR')
